...
async function generatePdf(data, outputPath) {
    return new Promise((resolve, reject) => {
        ...
        doc.fontSize(16).font("Helvetica-Bold").text(data.nome || "Roteiro", doc.page.margins.left + (data.logo_empresa_url ? 150 : 0), doc.page.margins.top, { align: "center" });
        doc.fontSize(10).font("Helvetica").text(`Tipo: ${data.tipo_roteiro || "N/A"} | Ano: ${data.ano || "N/A"} | Mês: ${data.mes || "N/A"} | Criado em: ${data.data_criacao_documento ? new Date(data.data_criacao_documento).toLocaleDateString("pt-BR") : "N/A"}`, { align: "center" });
        doc.moveDown(2);
        y = tableTop;
        ...
        const drawTableHeader = () => {
            ...
            doc.rect(x, y, pageWidth, headerHeight).fill("#EEEEEE");
        ...
        data.cenas.forEach((cena, index) => {
            ...
            doc.rect(x, y, pageWidth, maxHeight).stroke("#DDDDDD");
            ...
            data.colunas.forEach(col => {
                ...
                if (cena.type === "divisoria" && col.field === data.colunas[0].field) {
                    ...
                    doc.text(cena.nome_divisao || "DIVISÃO", x + 5, y + (maxHeight / 2) - 5, { 
                        width: pageWidth - 10, 
                        align: "center" 
                    });
